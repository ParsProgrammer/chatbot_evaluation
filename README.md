...
Unlike minimal implementations that only report intent accuracy and pass rate, this tool evaluates across multiple dimensions.
...
Evaluates response meaning rather than exact wording.

This accounts for LLM variability while still enforcing quality.
...
A test fails only if a turn fails in the majority of runs, preventing false regressions due to randomness.

📌 Assumptions Made

/chat endpoint follows contract:
...
Decision	Trade-off
Sequential execution	Simpler & deterministic but slower than parallel
Keyword + semantic validation	Flexible but more computational overhead
Majority-of-runs logic	Robust against noise but slightly masks rare failures
